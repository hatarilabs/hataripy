...
class PlotUtilities(object):
    ...
    which hataripy and hataripy6 can use to generate map based plots
    """
    ...
    def _plot_simulation_helper(simulation, model_list,
        ...
        Parameters
        ----------
        simulation : hataripy.mf6.Simulation object
        model_list : list
    ...
    def _plot_package_helper(package, **kwargs):
        ...
        Parameters
        ----------
        package: hataripy.pakbase.Package
        ...
        **kwargs : dict
        ...
        for item, value in package.__dict__.items():
            caxs = []
            # trap non-hataripy specific data_types.
    ...
    def _plot_mflist_helper(mflist, key=None, names=None, kper=0,
        ...
        Parameters
        ----------
        mflist: hataripy.utils.util_list.MfList object
    ...
    def _plot_util2d_helper(util2d, title=None, filename_base=None,
        ...
        Parameters
        ----------
        util2d : hataripy.util.util_array.Util2d object
        title : str
    ...
    def _plot_util3d_helper(util3d, filename_base=None,
        ...
        Parameters
        ----------
        util3d : hataripy.util.util_array.Util3d object
        filename_base : str
            Base file name that will be used to automatically generate file
            names for output image files. Plots will be exported as image
            files if file_name_base is not None. (default is None)
    ...
    def _plot_transient2d_helper(transient2d, filename_base=None,
        ...
        Parameters
        ----------
        transient2d : hataripy.utils.util_array.Transient2D object
        filename_base : str
            Base file name that will be used to automatically generate file
            names for output image files. Plots will be exported as image
            files if file_name_base is not None. (default is None)
    ...
    def _plot_array_helper(plotarray, model=None, modelgrid=None, axes=None,
        ...
        modelgrid: fp.discretization.ModelGrid object
            ...
            grid within hataripy. Optional if model object is provided
    ...
    def _plot_bc_helper(package, kper,
                        ...
                        mflay=None, **kwargs):
        """
        Helper method to plot bc objects from hataripy packages

        Parameters
        ----------
        package : hataripy.pakbase.Package objects
        kper : int
            zero based stress period number
        axes: matplotlib.axes object
            existing matplotlib axis object to layer additional
        ...
        **kwargs:
        ...
        # hataripy-modflow vs. hataripy-modflow6 trap
        try:
...
class SwiConcentration():
    ...
    def calc_conc(self, zeta, layer=None):
        ...
        >>> import hataripy
        >>> m = hataripy.modflow.Modflow.load('test')
        >>> c = hataripy.plot.SwiConcentration(model=m)
...
def shapefile_extents(shp):
    ...
    >>> import hataripy
    >>> fshp = 'myshapefile'
    >>> extent = hataripy.plot.plotutil.shapefile_extents(fshp)
...
def shapefile_get_vertices(shp):
    ...
    >>> import hataripy
    >>> fshp = 'myshapefile'
    >>> lines = hataripy.plot.plotutil.shapefile_get_vertices(fshp)
...
def findrowcolumn(pt, xedge, yedge):
    ...
    --------
    >>> import hataripy
    >>> irow, jcol = hataripy.plotutil.findrowcolumn(pt, xedge, yedge)
...
def line_intersect_grid(ptsin, xedge, yedge, returnvertices=False):
    ...
    --------
    >>> import hataripy
    >>> ptsout = hataripy.plotutil.line_intersect_grid(ptsin, xedge, yedge)
...
def cell_value_points(pts, xedge, yedge, vdata):
    ...
    --------
    >>> import hataripy
    >>> vcell = hataripy.plotutil.cell_value_points(xpts, xedge, yedge, head[0, :, :])
