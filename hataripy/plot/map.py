...
class PlotMapView(object):
    ...
    Parameters
    ----------
    modelgrid : hataripy.discretiztion.Grid
    ...
    def plot_bc(self, ftype=None, package=None, kper=0, color=None,
        ...
        type from a hataripy model

        Parameters
        ----------
        ftype : string
            Package name string ('WEL', 'GHB', etc.). (Default is None)
        package : hataripy.modflow.Modflow package class instance
            hataripy package class instance. (Default is None)
        kper : int
                ...
                if isinstance(travel_time, str):
                    if '<=' in travel_time:
                        ...
                        try:
                            ...
                            errmsg = 'hataripy.map.plot_pathline travel_time ' + \
    ...
    def plot_endpoint(self, ep, direction='ending',
        ...
        if direction == 'starting':
            ...
            errmsg = 'hataripy.map.plot_endpoint direction must be "ending" ' + \
                     'or "starting".'
            raise Exception(errmsg)
        ...
        if selection_direction is not None:
            if selection_direction.lower() != 'starting' and \
                ...
                errmsg = 'hataripy.map.plot_endpoint selection_direction ' + \
...
class DeprecatedMapView(PlotMapView):
    ...
    Parameters
    ----------
    model : hataripy.modflow.Modflow object
    modelgrid : hataripy.discretization.Grid object
    ...
    def plot_discharge(self, frf, fff, dis=None,
        ...
        fff : numpy.ndarray
            MODFLOW's 'flow front face'
        dis : hataripy.modflow.ModflowDis package
...
class ModelMap(object):
    ...
    Parameters
    ----------
    sr : hataripy.utils.reference.SpatialReference
    ...
    model : hataripy.modflow object
        hataripy model object. (Default is None)
    dis : hataripy.modflow.ModflowDis object
        hataripy discretization object. (Default is None)
    layer : int
        Layer to plot.  Default is 0.  Must be between 0 and nlay - 1.
    xul : float
