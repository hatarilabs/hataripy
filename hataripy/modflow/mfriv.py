...
the ModflowRiv class as `hataripy.modflow.ModflowRiv`.
...
class ModflowRiv(Package):
    ...
    stress_period_data : list of boundaries, or recarray of boundaries, or
        ...
        can be found in the hataripy3 boundaries Notebook in the basic
    ...
    mxactr : int
        ...
        automatically by hataripy based on the information in
    ...
    >>> riv = hataripy.modflow.ModflowRiv(m, stress_period_data=lrcd)
    ...
    def load(f, model, nper=None, ext_unit_dict=None, check=True):
        ...
        >>> import hataripy
        >>> m = hataripy.modflow.Modflow()
        >>> riv = hataripy.modflow.ModflowRiv.load('test.riv', m)
