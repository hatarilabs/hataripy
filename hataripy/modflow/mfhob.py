...
class ModflowHob(Package):
    ...
    >>> import hataripy
    >>> model = hataripy.modflow.Modflow()
    >>> dis = hataripy.modflow.ModflowDis(model, nlay=1, nrow=11, ncol=11, nper=2,
    ...                                perlen=[1,1])
    >>> tsd = [[1.,54.4], [2., 55.2]]
    >>> obsdata = hataripy.modflow.HeadObservation(model, layer=0, row=5,
    ...                                         column=5, time_series_data=tsd)
    >>> hob = hataripy.modflow.ModflowHob(model, iuhobsv=51, hobdry=-9999.,
    ...                                obs_data=obsdata)
    ...
    def load(f, model, ext_unit_dict=None, check=True):
        ...
        >>> hobs = hataripy.modflow.ModflowHob.load('test.hob', m)
