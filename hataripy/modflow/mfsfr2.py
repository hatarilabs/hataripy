...
class ModflowSfr2(Package):
    ...
    reach_data : recarray
        ...
        following hataripy convention, layer, row, column and node number
    ...
    >>> sfr2 = hataripy.modflow.ModflowSfr2(ml, ...)
    ...
    nsfrpar = 0
    heading = '# Streamflow-Routing (SFR2) file for MODFLOW, generated by hataripy'
    default_value = 0.
    ...
    def assign_layers(self, adjust_botms=False, pad=1.):
        ...
        of the hataripy.model.ModflowDis instance. To produce a
        new DIS package file, model.write() or hataripy.model.ModflowDis.write()
        must be run.
        ...
        if np.any(below):
            ...
            if not adjust_botms:
                ...
                print('New bottom array assigned to hataripy DIS package '
                      'instance.\nRun hataripy.model.write() or '
                      'hataripy.model.ModflowDis.write() to write new DIS file.')
            header += '\n'
    ...
    def deactivate_ibound_above(self):
        ...
        Notes
        -----
        This routine updates the ibound array of the hataripy.model.ModflowBas6
        instance. To produce a new BAS6 package file, model.write() or
        hataripy.model.ModflowBas6.write() must be run.
    ...
    def export(self, f, **kwargs):
        if isinstance(f, str) and f.lower().endswith(".shp"):
            from hataripy.utils.geometry import Polygon
    ...
    def export_outlets(self, f, **kwargs):
        ...
        """
        from hataripy.utils.geometry import Point
        from hataripy.export.shapefile_utils import recarray2shp
        rd = self.reach_data
...
class check:
    ...
    ----------
    sfrpackage : object
        Instance of hataripy ModflowSfr2 class.
    verbose : bool
        Boolean flag used to determine if check method results are
        written to the screen
    level : int
    ...
    def overlapping_conductance(self, tol=1e-6):
        ...
        # always use unique rc, since hataripy assigns nodes by k, i, j
        uniquerc = {}
