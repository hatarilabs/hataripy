...
the ModflowDrt class as `hataripy.modflow.ModflowDrt`.
...
class ModflowDrt(Package):
    ...
    stress_period_data : list of boundaries, recarrays, or dictionary of
        ...
        stress_period_data can be found in the hataripy3boundaries Notebook in
    ...
    >>>                                   #stress periods
    >>> drt = hataripy.modflow.ModflowDrt(ml, stress_period_data=lrcec)
    ...
    def load(f, model, nper=None, ext_unit_dict=None, check=True):
        ...
        >>> drn = hataripy.modflow.ModflowDrt.load('test.drt', m)
