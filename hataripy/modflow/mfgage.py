...
the ModflowGage class as `hataripy.modflow.ModflowGage`.
...
class ModflowGage(Package):
    ...
    >>> gage = hataripy.modflow.ModflowGage(m, numgage=2,
    ...
    def write_file(self):
        ...
        # self.heading = '# {} package for '.format(self.name[0]) + \
        #                '{}, generated by hataripy.'.format(vn)
        # f.write('{0}\n'.format(self.heading))
    ...
    def load(f, model, nper=None, ext_unit_dict=None):
        ...
        >>> import hataripy
        >>> m = hataripy.modflow.Modflow()
        >>> gage = hataripy.modflow.ModflowGage.load('test.gage', m)
