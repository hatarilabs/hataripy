"""
Module for exporting and importing hataripy model attributes
"""
import copy
import shutil
import json
...
def model_attributes_to_shapefile(filename, ml, package_names=None,
    ...
    >>> hataripy.utils.model_attributes_to_shapefile('model.shp', m)
    ...
    if grid.grid_type == 'USG-Unstructured':
        raise Exception('hataripy does not support exporting to shapefile from '
...
def shape_attr_name(name, length=6, keep_layer=False):
    ...
    >>> import hataripy
    >>> name = hataripy.utils.shape_attr_name('averylongstring')
    >>> name
    >>> 'averyl'
...
def recarray2shp(recarray, geoms, shpname='recarray.shp', mg=None,
    ...
    recarray : np.recarray
        ...
        shapefile
    geoms : list of hataripy.utils.geometry objects
    ...
    subsequent use. See hataripy.reference for more details.
    ...
    if geomtype == shapefile.POLYGON:
        ...
        # hataripy, even if the point is 3D.
        if sfv < 2:
            for i, r in enumerate(ralist):
...
class CRS(object):
    ...
    def get_spatialreference(epsg, text='esriwkt'):
        ...
        """
        from hataripy.utils.flopy_io import get_url_text
...
class EpsgReference:
    ...
    def __init__(self):
        try:
            ...
            user_data_dir = None
        if user_data_dir:
            datadir = user_data_dir('hataripy')
        else:
            # if appdirs is not installed, use user's home directory
            datadir = os.path.join(os.path.expanduser('~'), '.hataripy')
