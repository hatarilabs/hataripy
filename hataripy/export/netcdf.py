...
class NetCdf(object):
    """
    Support for writing a netCDF4 compliant file from a hataripy model
    ...
    def _initialize_attributes(self):
        ...
        for n, v in spatial_attribs.items():
            self.global_attributes["hataripy_sr_" + n] = v
    ...
    def initialize_file(self, time_values=None):
        ...
        self.nc.setncattr("Conventions", "CF-1.6, ACDD-1.3, hataripy {}".format(
            hataripy.__version__))
        ...
        crs = hataripy.utils.reference.crs(prj=self.model_grid.prj,
    ...
    def add_sciencebase_metadata(self, id, check=True):
        ...
        hataripy.export.metadata.acdd class.

        Returns
        -------
        metadata : hataripy.export.metadata.acdd object
        """
    ...
    def _check_vs_sciencebase(self, md):
        """Check that model bounds read from hataripy are consistent with those in ScienceBase."""
    ...
    def get_longnames_from_docstrings(self, outfile='longnames.json'):
        ...
        Scrape hataripy module docstrings and return docstrings for parameters
        ...
        # get a list of the hataripy classes
        # packages = inspect.getmembers(hataripy.modflow, inspect.isclass)
