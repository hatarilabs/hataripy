...
hataripypth = os.path.join(thisfilepath, '..', '..')
hataripypth = os.path.abspath(hataripypth)
print('hataripy is installed in {}'.format(hataripypth))
protected_dfns = ['hataripy.dfn']
...
def backup_existing_dfns(hataripy_dfn_path):
    parent_folder = os.path.dirname(hataripy_dfn_path)
    timestr = time.strftime("%Y%m%d-%H%M%S")
    backup_folder = os.path.join(parent_folder, 'dfn_backup', timestr)
    shutil.copytree(hataripy_dfn_path, backup_folder)
    assert os.path.isdir(backup_folder), \
        'dfn backup files not found: {}'.format(pth)
    return


def replace_dfn_files(new_dfn_pth, hataripy_dfn_path):

    # remove the old files, unless the file is protected
    filenames = os.listdir(hataripy_dfn_path)
    delete_files(filenames, hataripy_dfn_path, exclude=protected_dfns)
    ...
    for filename in filenames:
        ...
        shutil.copy(filename_w_path, hataripy_dfn_path)


def delete_mf6_classes():
    pth = os.path.join(hataripypth, 'mf6', 'modflow')
    files = [entry for entry in os.listdir(pth) if
             os.path.isfile(os.path.join(pth, entry))]
...
def generate_classes(branch='master', dfnpath=None, backup=True):
    """
    Generate the MODFLOW 6 hataripy classes using definition files from the
    ...
    print(2 * '\n')
    print(72 * '*')
    print('Updating the hataripy MODFLOW 6 classes')
    hataripy_dfn_path = os.path.join(hataripypth, 'mf6', 'data', 'dfn')

    # download the dfn files and put them in hataripy.mf6.data or update using
    # user provided dfnpath
    if dfnpath is None:
        print('  Updating the MODFLOW 6 classes using the branch: {}'.format(branch))
        timestr = time.strftime("%Y%m%d-%H%M%S")
        new_dfn_pth = os.path.join(hataripypth, 'mf6', 'data', 'dfn_' + timestr)
    ...
    if backup:
        print('  Backup existing definition files in: {}'.format(hataripy_dfn_path))
        backup_existing_dfns(hataripy_dfn_path)

    print('  Replacing existing definition files with new ones.')
    replace_dfn_files(new_dfn_pth, hataripy_dfn_path)
    if dfnpath is None:
    ...
    create_packages()
    list_files(os.path.join(hataripypth, 'mf6', 'modflow'))
