...
class MFBlockHeader(object):
    ...
    def __init__(self, name, variable_strings, comment, simulation_data=None,
        ...
        if not ((simulation_data is None and path is None) or
            ...
            raise hataripyException('Block header must be initialized with both '
                ...
                if isinstance(transient_key, np.recarray):
                    ...
                    if not (key_index is not None and
                        ...
                        if key_index is None:
                            raise hataripyException('Block header index could '
                                                 'not be determined.')
                        else:
                            raise hataripyException('Block header index "{}" '
                                                 'must be less than "{}"'
                                                 '.'.format(
...
class MFPackage(PackageContainer, PackageInterface):
    ...
    def plot(self, **kwargs):
        ...
        Parameters
        ----------
        package: hataripy.pakbase.Package instance supplied for plotting
