...
from ..mfbase import StructException, hataripyException
    ...
    def get_model_dim(self, data_item_num):
        if self.package_dim.model_dim is None or data_item_num is None or \
            ...
            if not (len(self.structure.data_item_structures) >
                    data_item_num):
                raise hataripyException('Data item index "{}" requested which '
...
class ModelDimensions(object):
    ...
    def get_data_shape(self, structure, data_item=None, data_set_struct=None,
        ...
        if structure is None:
            raise hataripyException('get_data_shape requires a valid structure '
                                 'object')
        if self.locked:
