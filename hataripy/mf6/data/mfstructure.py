...
class Dfn(object):
    ...
    def get_file_list(self):
        ...
        for f in files:
            if 'common' in f or 'hataripy' in f:
                continue
...
class MFDataStructure(object):
    ...
    def _fpmerge_data_item(self, item, dfn_list):
        ...
        # check for hataripy-specific dfn data
        if item.name.lower() in mfstruct.hataripy_dict:
            # read hataripy-specific dfn data
            for name, value in mfstruct.hataripy_dict[item.name.lower()].items():
...
class MFStructure(object):
    ...
    def __load_structure(self):
        ...
        if self.load_from_dfn_files:
            ...
            # load hataripy-specific settings
            self.__load_hataripy()
    ...
    def __load_hataripy(self):
        current_variable = None
        var_info = {}
        dfn_path, tail = os.path.split(os.path.realpath(__file__))
        hataripy_path = os.path.join(dfn_path, 'dfn', 'hataripy.dfn')
        dfn_fp = open(hataripy_path, 'r')
        for line in dfn_fp:
            if self.__valid_line(line):
                lst_line = line.strip().split()
                if lst_line[0].lower() == 'name':
                    # store current variable
                    self.hataripy_dict[current_variable] = var_info
                    # reset var_info dict
                    var_info = {}
