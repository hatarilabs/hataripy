"""
The hataripy package consists of a set of Python scripts to run MODFLOW, MT3D,
SEAWAT and other MODFLOW-related groundwater programs. hataripy enables you to
run all these programs with Python scripts. The hataripy project started in 2009
and has grown to a fairly complete set of scripts with a growing user base.

This version of hataripy (hataripy3) was released in December 2015 with a few great
enhancements that make hataripy3 backwards incompatible. The first significant
change is that hataripy3 uses zero-based indexing everywhere, which means that
...
specify these boundary conditions. hataripy is an open-source project and any
