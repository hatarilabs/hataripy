...
class SeawatVsc(Package):
    ...
    ----------
    model : model object
        The model object (of type :class:`hataripy.seawat.swt.Seawat`) to which
    ...
    >>> import hataripy
    >>> m = hataripy.seawat.Seawat()
    >>> vsc = hataripy.modflow.SeawatVsc(m)
    ...
    def load(f, model, nper=None, ext_unit_dict=None):
        ...
        f : filename or file handle
            File to load.
        model : model object
            The model object (of type :class:`hataripy.seawat.swt.Seawat`) to
        ...
        >>> import hataripy
        >>> mf = hataripy.modflow.Modflow()
        >>> dis = hataripy.modflow.ModflowDis(mf)
        >>> mt = hataripy.mt3d.Mt3dms()
        >>> swt = hataripy.seawat.Seawat(modflowmodel=mf, mt3dmsmodel=mt)
        >>> vdf = hataripy.seawat.SeawatVsc.load('test.vsc', m)
