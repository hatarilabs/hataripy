...
class Mt3dSft(Package):
    ...
    >>> mf = hataripy.modflow.Modflow.load('CrnkNic_mf.nam',
    >>>                                  load_only=['dis', 'bas6'])
    >>> sfr = hataripy.modflow.ModflowSfr2.load('CrnkNic.sfr2', mf)
    >>> chk = sfr.check()

    >>> # initialize an MT3D-USGS model
    >>> mt = hataripy.mt3d.Mt3dms.load('CrnkNic_mt.nam',
    >>>        exe_name = 'mt3d-usgs_1.0.00.exe',
    >>>        model_ws = r'.\CrnkNic',
    >>>        load_only='btn')
    >>> sft = hataripy.mt3d.Mt3dSft.load(mt, 'CrnkNic.sft')
    ...
    def load(f, model, nsfinit=None, nper=None, ncomp=None,
        ...
        ...                                 load_only=['dis', 'bas6'])
        >>> sfr = hataripy.modflow.ModflowSfr2.load('CrnkNic.sfr2', mf)
        >>> mt = hataripy.mt3d.Mt3dms.load('CrnkNic_mt.nam', load_only='btn')
        >>> sft = hataripy.mt3d.Mt3dSft.load('CrnkNic.sft', mt)
